...
struct ColorValue
    : pegtl::sor<TAO_PEGTL_STRING("red"), TAO_PEGTL_STRING("blue"),
                 TAO_PEGTL_STRING("green")> {};

struct CubeSetItem
    : public pegtl::seq<pegtl::blank, ColorCount, pegtl::blank, ColorValue> {};

struct Line
    : public pegtl::seq<TAO_PEGTL_STRING("Game "), GameNumber, pegtl::one<':'>,
...
template <> struct Action<GameNumber> {
...
template <> struct Action<ColorCount> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, Games &games) {
    assert(!games.empty());
...
template <> struct Action<ColorValue> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, Games &games) {
    assert(!games.empty());
    auto &sets = games.back().sets;
...
template <> struct Control<Line> : pegtl::normal<Line> {
  template <typename ParseInput> static void start(ParseInput &, Games &games) {
    games.emplace_back();
  }
};

template <> struct Control<CubeSetItem> : pegtl::normal<CubeSetItem> {
  template <typename ParseInput> static void start(ParseInput &, Games &games) {
    assert(!games.empty());
...
MaxCount getMaxCount(const CubeSets &sets) {
  auto getMaxForColor = [&](Color color) {
    auto filtered = sets | vw::filter([&](const CubeSet &set) {
  ...
  return {.red = getMaxForColor(Color::red),
...
int solvePart1(std::string_view input) {
  ...
  return std::ranges::fold_left(games | vw::filter([](const Game &game) {
                                  const auto max = getMaxCount(game.sets);
                                  return max.red <= 12 && max.green <= 13 &&
                                         max.blue <= 14;
...
int solvePart2(std::string_view input) {
  const auto games = Parsing::parse(input);
  return std::ranges::fold_left(games | vw::transform([](const Game &game) {
                                  const auto max = getMaxCount(game.sets);
