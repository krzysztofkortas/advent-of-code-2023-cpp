  ...
  for (int i = 0; i < steps; ++i) {
    ...
    for (const auto &plot : plots) {
      for (const auto &dir : {Direction{-1, 0}, {1, 0}, {0, -1}, {0, 1}}) {
        const Position newPlot{plot.first + dir.first,
                               plot.second + dir.second};
        if (std::min(newPlot.first, newPlot.second) < 0 ||
            newPlot.first > height || newPlot.second > width) {
          continue;
        }
...
int64_t solvePart2(std::string_view input, int64_t steps) {
  ...
  answers = {answers[modulo], answers[modulo + width],
             answers[modulo + 2 * width]};
  ...
  while (answersSize < neededSteps) {
    answers.push_back(diff + 2 * answers[answersSize - 1] -
                      answers[answersSize - 2]);
    ++answersSize;
  }
