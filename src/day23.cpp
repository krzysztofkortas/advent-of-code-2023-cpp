...
int64_t dfs(const Grid &grid, const Position &pos, const Position &end,
            const Directions &directions, Visited &visited) {
  if (pos == end)
    return 0;
  ...
  for (const Direction &dir : directions.at(c)) {
    ...
    distance =
        std::max(distance, dfs(grid, newPos, end, directions, visited) + 1);
