...
class Graph {
public:
  explicit Graph(Grid grid) : grid_{std::move(grid)} { makeConnections(); }
  ...
  Path getCycle(const Position &start) {
    ...
    do {
      ...
      for (const auto &direction : {Position{-1, 0}, {0, -1}, {0, 1}, {1, 0}}) {
        const Position newPos{currentPos.row + direction.row,
...
std::pair<Path, Grid> getMaxCycle(std::string_view input) {
  ...
  return std::ranges::max(
      ...
      std::less{}, [](const auto &p) { return p.first.size(); });
}
...
bool isOnCycle(const Position &pos, const Path &cycle) {
  return std::ranges::any_of(cycle, [&](const Connection &c) {
    return c.begin == pos || c.end == pos;
  });
}
