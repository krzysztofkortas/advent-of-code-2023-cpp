...
int64_t solvePart1(std::string_view input) {
  return std::ranges::fold_left(input | vw::split(',') | vw::transform(getHash),
                                0z, std::plus{});
}
...
Step makeStep(const std::string &step) {
  const static std::regex regex(R"((\w+)([-=])(\w*))");
  std::smatch match;
  if (std::regex_match(step, match, regex)) {
    return Step{.label = match.str(1),
                .operation = makeOperation(match.str(2).at(0)),
                .focalLength = match.str(3).empty()
                                   ? std::nullopt
                                   : std::optional{std::stoll(match.str(3))}};
  }
  std::unreachable();
}
...
int64_t solvePart2(std::string_view input) {
  const Steps steps = input | vw::split(',') | vw::transform([](auto &&step) {
                        return makeStep(step | std::ranges::to<std::string>());
                      }) |
                      std::ranges::to<Steps>();
