...
struct Line : public pegtl::seq<TAO_PEGTL_STRING("Card"), Separator, CardNumber,
                                pegtl::one<':'>, Separator,
                                pegtl::list<WinningNumber, Separator>,
                                Separator, pegtl::one<'|'>, Separator,
                                pegtl::list<Number, Separator>, pegtl::eolf> {};

struct File : pegtl::must<pegtl::until<pegtl::eof, Line>> {};

template <typename Rule> struct Action : pegtl::nothing<Rule> {};

template <> struct Action<CardNumber> {
...
template <> struct Action<WinningNumber> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, Cards &cards) {
    assert(!cards.empty());
...
template <> struct Control<Line> : pegtl::normal<Line> {
  template <typename ParseInput> static void start(ParseInput &, Cards &cards) {
    cards.emplace_back();
  }
};
...
int64_t solvePart1(std::string_view input) {
  ...
  return std::ranges::fold_left(cards | vw::transform([](const Card &card) {
                                  const int intersectionSize =
                                      getIntersectionSize(card.winningNumbers,
                                                          card.numbers);
...
int64_t solvePart2(std::string_view input) {
  ...
  for (const auto &card : cards) {
    ...
    for (int64_t i = card.id;
         i < std::min(card.id + intersectionSize, ssize(cards)); ++i) {
