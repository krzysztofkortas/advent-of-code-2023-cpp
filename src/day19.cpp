...
struct RuleRule
    : pegtl::seq<pegtl::opt<ConditionRule, pegtl::one<':'>>, ConsequentRule> {};
struct WorkflowRule
    : pegtl::seq<WorkflowNameRule, pegtl::one<'{'>,
                 pegtl::list<RuleRule, Separator>, pegtl::one<'}'>> {};

struct RatingCategoryRule : CategoryRule {};
struct RatingValueRule : ValueRule {};
struct RatingItemRule
    : pegtl::seq<RatingCategoryRule, pegtl::one<'='>, RatingValueRule> {};
struct RatingRule
    : pegtl::seq<pegtl::one<'{'>, pegtl::list<RatingItemRule, Separator>,
                 pegtl::one<'}'>> {};

struct Grammar
    : pegtl::must<pegtl::list<WorkflowRule, pegtl::eol>, pegtl::eol, pegtl::eol,
                  pegtl::list<RatingRule, pegtl::eol>, pegtl::eof> {};

template <typename Rule> struct Action : pegtl::nothing<Rule> {};

template <> struct Action<WorkflowNameRule> {
...
template <> struct Action<CategoryRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
template <> struct Action<OperatorRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
template <> struct Action<ValueRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
template <> struct Action<RuleRule> {
  static void apply0(State &state) {
...
template <> struct Action<WorkflowRule> {
  static void apply0(State &state) {
...
template <> struct Action<RatingCategoryRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
template <> struct Action<RatingValueRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
template <> struct Action<RatingItemRule> {
  static void apply0(State &state) {
    state.tempRating[state.tempRatingCategory] = state.tempRatingValue;
  }
};

template <> struct Action<RatingRule> {
  static void apply0(State &state) {
...
bool isWorkflowAccepted(const Workflow &workflow, const Workflows &workflows,
                        const Rating &rating) {
...
int64_t solvePart1(std::string_view input) {
  ...
  return std::ranges::fold_left(ratings | vw::filter([&](const Rating &rating) {
                                  return isWorkflowAccepted(startWorkflow,
                                                            workflows, rating);
...
std::pair<RatingRange, RatingRange> crossCondition(const Condition &condition,
                                                   const RatingRange &ratings) {
  const auto [range1, range2] = [&]() -> std::pair<Range, Range> {
    const auto range = ratings.at(condition.category);
    switch (condition.op) {
    case Operator::less:
      return {Range{range.start, condition.value},
              Range{condition.value, range.end}};
    case Operator::greater:
      return {Range{condition.value + 1, range.end},
...
int64_t countRatings(const RatingRange &ratings) {
  return std::ranges::fold_left(ratings | vw::values |
                                    vw::transform([](const Range &range) {
                                      return range.end - range.start;
                                    }),
                                1, std::multiplies{});
}

int64_t countAccepted(const Workflow &workflow, const Workflows &workflows,
                      RatingRange ratings) {
  int64_t result = 0;
  for (const Rule &rule : workflow.rules) {
    ...
    if (!rule.condition) {
      ...
      const auto [ratings1, ratings2] =
          crossCondition(*rule.condition, ratings);
