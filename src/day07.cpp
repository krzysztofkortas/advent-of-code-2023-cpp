...
template <> struct Action<CardsRule> {
...
template <> struct Action<BidRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
template <> struct Action<LineRule> {
  static void apply0(State &state) { state.hands.push_back(state.currentHand); }
};
...
HandType getHandType(const Hand &hand) {
  ...
  auto count = hand.cards | vw::filter([](char c) { return c != 'J'; }) |
               vw::transform([&](char c) {
                 return std::make_pair(c, std::ranges::count(hand.cards, c));
               }) |
               std::ranges::to<std::map>() | vw::values |
...
std::vector<int> getCardValues(const std::string &cards) {
  ...
  return cards | vw::transform([=](char c) {
           return static_cast<int>(orderedCards.find(c));
         }) |
         std::ranges::to<std::vector>();
}
...
int64_t getOverallRating(Hands hands) {
  ...
  return std::ranges::fold_left(hands | vw::enumerate |
                                    vw::transform([](const auto &p) {
