...
struct SeedsRule : pegtl::seq<TAO_PEGTL_STRING("seeds: "),
                              pegtl::list<SeedRule, pegtl::blank>> {};
...
struct MapItemRule
    : pegtl::seq<DestinationRangeStartRule, pegtl::blank, SourceRangeStartRule,
                 pegtl::blank, RangeLengthRule> {};
struct MapRule : pegtl::seq<MapNameRule, pegtl::eol,
                            pegtl::list<MapItemRule, pegtl::eol>> {};

struct Grammar
    : pegtl::must<SeedsRule,
                  pegtl::until<pegtl::eof, pegtl::pad<MapRule, pegtl::space>>> {
};

template <typename Rule> struct Action : pegtl::nothing<Rule> {};

template <> struct Action<SeedRule> {
...
template <> struct Action<DestinationRangeStartRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
template <> struct Action<SourceRangeStartRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
template <> struct Action<RangeLengthRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
int64_t solvePart1(std::string_view input) {
  ...
  return std::ranges::min(
      seeds | vw::transform([&](int64_t seed) {
...
SeedRanges convert(const SeedRanges &seedRanges, const Map &map) {
  ...
  for (const SeedRange &seedRange : seedRanges) {
    ...
    for (const MapItem &mapItem : map) {
      const int64_t diff =
          mapItem.destinationRangeStart - mapItem.sourceRangeStart;
      const int64_t rangeStart =
          std::max(seedRange.start, mapItem.sourceRangeStart);
      const int64_t rangeEnd = std::min(
          seedRange.end, mapItem.sourceRangeStart + mapItem.rangeLength);
...
int64_t solvePart2(std::string_view input) {
  ...
  SeedRanges seedRanges =
      seeds | vw::chunk(2) | vw::transform([](auto &&range) {
        assert(range.size() == 2);
