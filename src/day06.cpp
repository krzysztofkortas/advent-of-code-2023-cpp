...
struct TimeLineRule
    : pegtl::seq<TAO_PEGTL_STRING("Time:"), SeparatorRule,
                 pegtl::list<TimeRule, SeparatorRule>, pegtl::eol> {};
struct DistanceLineRule
    : pegtl::seq<TAO_PEGTL_STRING("Distance:"), SeparatorRule,
                 pegtl::list<DistanceRule, SeparatorRule>, pegtl::eolf> {};

struct Grammar : pegtl::must<TimeLineRule, DistanceLineRule> {};

template <typename Rule> struct Action : pegtl::nothing<Rule> {};

template <> struct Action<TimeRule> {
...
template <> struct Action<DistanceRule> {
  template <typename ActionInput>
  static void apply(const ActionInput &in, State &state) {
...
Races parsePart1(std::string_view input) {
  ...
  return vw::zip(state.times, state.distances) | vw::transform([](auto &&elem) {
           return Race{.time = std::get<0>(elem),
                       .distance = std::get<1>(elem)};
         }) |
...
Race parsePart2(std::string_view input) {
  ...
  constexpr auto joinValues =
      vw::transform([](int64_t val) { return std::to_string(val); }) |
      vw::join | std::ranges::to<std::string>();
  return Race{.time = std::stoll(state.times | joinValues),
...
int64_t solvePart1(std::string_view input) {
  ...
  return std::ranges::fold_left(races | vw::transform(getWinnings), 1,
                                std::multiplies<>{});
}
