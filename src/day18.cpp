...
int64_t solvePart1(std::string_view input) {
  ...
  for (auto &&line : input | vw::split('\n')) {
    const auto v =
        line | vw::split(' ') | std::ranges::to<std::vector<std::string>>();
    assert(v.size() == 3);
    const Direction dir = getDir(v[0].front());
    const int64_t meters = std::stoll(v[1]);
    circuit += meters;
    const Position currentPoint{previousPoint.first + meters * dir.first,
                                previousPoint.second + meters * dir.second};
    area += previousPoint.first * currentPoint.second -
            previousPoint.second * currentPoint.first;
